...
export default function App() {
  ...
  const handleContentEditableChange = (e) => {
    if (e.keyCode == "8") {
      ...
      if (window.navigator.userAgent.match(/firefox|fxios\/([0-9]+)\./i)) {
        if (window.getSelection().anchorNode.textContent != window.getSelection().focusNode.textContent == '') {
          let childNodesLength = document.getElementById("textid").childNodes.length;
          if (childNodesLength > 1) {
            for (let index = childNodesLength; index > 1; index--) {
              document.getElementById("textid").childNodes[index - 1].remove();
            }
          }
          if (window.getSelection().anchorOffset === 0) {
            debugger
            document.getElementById("textid").querySelectorAll('span')[0].innerHTML = "<br>";
            window.getSelection().anchorNode.parentNode.parentNode.getElementsByTagName("span")[0].removeAttribute("last-data");
            window.getSelection().anchorNode.parentNode.parentNode.getElementsByTagName("span")[0].setAttribute("data-content", '');
            window.getSelection().anchorNode.parentNode.parentNode.getElementsByTagName("span")[0].removeAttribute("data-content"); e.preventDefault();
            return false;
          }
          if (window.getSelection().anchorOffset === 0) {
        ...
        if (window.getSelection().anchorNode.textContent != window.getSelection().focusNode.textContent) {
          ...
          if (window.getSelection().anchorOffset === 0) {
            ...
            window.getSelection().anchorNode.parentNode.parentNode.getElementsByTagName("span")[0].removeAttribute("last-data");
            window.getSelection().anchorNode.parentNode.parentNode.getElementsByTagName("span")[0].setAttribute("data-content", '');
            window.getSelection().anchorNode.parentNode.parentNode.getElementsByTagName("span")[0].removeAttribute("data-content");
            e.preventDefault();
            return false;
          }
        }
    ...
    if (e.keyCode == "8") {
      if (document.getElementById("textid").querySelectorAll('div').length < 2) {
        ...
        else if (document.getElementById("textid").querySelectorAll('voice')[0].innerHTML.length == 215) {
          document.getElementById("textid").querySelectorAll('span')[0].innerHTML = "<br>";
          e.preventDefault();
          return false;
        }
  ...
  function addRate(tag, rate, value) {
    ...
    if (window.getSelection().toString() == '') {
      return swal({
        text: "Make sure you've selected text !",
        button: "X",
        dangerMode: true,
      })
    }
      ...
      firstpart = explode[0];
      if(window.getSelection().anchorNode.parentNode.firstElementChild != null)
      {
        if( window.getSelection().anchorNode.parentNode.firstElementChild.nodeName =='BREAK')
        {
              
          finaltag = '<prosody id="prosody" class="rate" data-name="' + value + "%" + '">' + firstpart + '</prosody>';
